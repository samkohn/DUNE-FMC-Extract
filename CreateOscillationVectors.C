        ...
        const double EMAX)
{
    ...
    */
    ...
    std::string foutprefix = CFG_OutputDir + CFG_OscDir;
                ...
                for(dm21=dm21s.begin(); dm21 != dm21s.end(); ++dm21)
                {
                    for(dm31=dm31s.begin();dm31!=dm31s.end();++dm31)
                    {
                        for(dcp=dcps.begin(); dcp!=dcps.end();++dcp)
                        {
                            ++paramSetCount;
                            char foldername[100];
                            sprintf(foldername, "/oscvectors_%d/", paramSetCount);
                            std::string foutprefix = CFG_OutputDir + CFG_OscSetsDir + foldername;
